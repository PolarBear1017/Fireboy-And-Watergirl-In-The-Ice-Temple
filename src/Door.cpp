...
Door::Door(const std::shared_ptr<SpriteAtlas> &atlas, const Element element,
           const glm::vec2 &pos)
    : m_Element(element) {
  SetZIndex(5);

  m_Transform.translation = pos;
  m_Transform.translation.y -= 5.0f; // Lower the door to avoid floating
  m_Transform.scale = {0.65f, 0.65f};

  const std::string prefix =
      (m_Element == Element::FIRE) ? "FinishBoy" : "FinishGirl";
